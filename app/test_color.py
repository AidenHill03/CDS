"""test_color.py -- property-based checks for app/color.

Mirrors the rest of this project's test style (PASS/FAIL per check,
properties rather than golden images -- a color LUT is exactly the kind of
thing a pixel-diff test would rot on for no functional reason). Run with:

    PYTHONPATH=cdx/build python -m app.test_color
...
def main() -> None:
    ...
    # ---- color_escape_time: never-escaped gets a flat color, not palette index 0 --
    print("\ncolor_escape_time:")
    vals = np.array([0.0, 1.0, 50.0, 199.0])
    img = color_escape_time(vals, max_iter=200, palette="viridis")
    ...
    flat = color_basin(labels, iterations=None)
    check(tuple(flat[1]) == tuple(flat[2]),
          "with no iterations array, basin coloring is flat -- same basin id, same color "
          "regardless of position")

    all_unresolved = color_basin(np.zeros(5, dtype=int))
    check(np.all(all_unresolved == 0), "an all-unresolved label array is entirely black, no crash")

    # ---- color_basin: gradient scaling modes (Stage 0) ------------------------------
    print("\ncolor_basin: gradient scaling modes:")
    try:
        color_basin(labels, iters, max_iter=200, scaling="not-a-real-scaling")
        check(False, "an unknown scaling name raises")
    except ValueError as e:
        check("scaling" in str(e), "ValueError names the bad scaling mode, same as "
              "color_escape_time's own validation")
    ...
    for scaling in ("log1p", "histogram"):
        rimg = color_basin(rich_labels, rich_iters, max_iter=200, scaling=scaling)
    ...
    pimg = color_basin(period_labels, period_iters, max_iter=200, period=10.0)
    ...
    hist_hue_img = color_basin(labels, iters, max_iter=200, scaling="histogram")
    ...
    hist_flat = color_basin(labels, iterations=None, scaling="histogram")
    check(tuple(hist_flat[1]) == tuple(flat[1]),
          "with no iterations array, the scaling mode has no effect at all -- same flat "
          "color regardless of 'scaling'")

    # ---- color_parameter_basin: categorical by count, unresolved-dominant flat -----
    print("\ncolor_parameter_basin:")
    counts = np.array([0, 1, 1, 2, 3])
    pbimg = color_parameter_basin(counts)
    check(tuple(pbimg[0]) == PARAMETER_BASIN_UNRESOLVED_RGB,
          "count == 0 gets the flat unresolved color, not a 'count 0' hue of its own")
    check(tuple(pbimg[1]) == tuple(pbimg[2]),
          "the SAME count gets the SAME color, deterministically")
    check(tuple(pbimg[1]) != tuple(pbimg[3]) and tuple(pbimg[3]) != tuple(pbimg[4]) and
         tuple(pbimg[1]) != tuple(pbimg[4]),
          "three DIFFERENT counts (1, 2, 3) get three visually distinct colors")

    # Categorical, not a gradient: count=1 and count=2 aren't required to be
    # "close" in color space the way adjacent escape-time values would be
    ...
    no_unresolved = color_parameter_basin(np.array([1, 2]))
    check(tuple(no_unresolved[0]) == tuple(color_parameter_basin(np.array([1, 2]),
                                                                  np.array([0, 0]))[0]),
          "omitting `unresolved` entirely behaves the same as passing an all-zero array")

    dominant = color_parameter_basin(np.array([2, 2]), np.array([1, 3]))
    check(tuple(dominant[0]) != PARAMETER_BASIN_UNRESOLVED_RGB,
          ...
          "real count's color -- a confirmed attractor is shown as confirmed")
    check(tuple(dominant[1]) == PARAMETER_BASIN_UNRESOLVED_RGB,
          "count=2 with unresolved=3 (unresolved EXCEEDS count -- a majority) is "
          "UNRESOLVED-DOMINANT and gets the flat neutral color instead, not conflated "
          "with a real count")

    all_unresolved_pb = color_parameter_basin(np.zeros(4, dtype=int))
    ...
    # unit-test here beyond color_escape_time's own existing coverage above;
    ...
    # ---- color_scalar_field: every pixel colored, cyclic banding ------------------
    print("\ncolor_scalar_field:")
    field = np.array([1e-9, 1.0, np.e ** 12, np.e ** 24])
    fimg = color_scalar_field(field, band_width=1.0, period_bands=12.0)
    ...
    check(np.array_equal(fimg[1], fimg[3]),
          ...
          "color -- the cyclic equipotential-band wrap")

    try:
        color_scalar_field(field, palette="not-a-real-palette")
        check(False, "an unknown palette name raises")
    except ValueError:
        check(True, "color_scalar_field validates its palette argument too")

    # ---- color_scalar_field: contour lines ------------------------------------------
    print("\ncolor_scalar_field (contour lines):")
    ...
    without_contour = color_scalar_field(varying, band_width=1.0, period_bands=12.0)
    with_contour = color_scalar_field(varying, band_width=1.0, period_bands=12.0, contour=True)
    ...
    check(tuple(with_contour[0, 1]) == (0, 0, 0),
          "a pixel on a real band boundary is drawn in the contour color")

    flat_with_contour = color_scalar_field(flat, band_width=1.0, period_bands=12.0, contour=True)
    flat_without = color_scalar_field(flat, band_width=1.0, period_bands=12.0)
    ...
    custom_color = color_scalar_field(varying, band_width=1.0, period_bands=12.0, contour=True,
                                        contour_rgb=(255, 0, 0))
    check(tuple(custom_color[0, 1]) == (255, 0, 0),
          "contour_rgb controls the actual drawn color, not just an on/off flag")
